...
ubuntu16.04:  ubuntu16.04-validate
	packer build --on-error="cleanup" -only=virtualbox-iso -var-file=ubuntu-variables.json ubuntu_16.04.json
ubuntu1604-debug:  ubuntu16.04-validate
	PACKER_LOG=1 packer build -only=virtualbox-iso --on-error="cleanup" -var-file=ubuntu-variables.json ubuntu_16.04.json
